...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'your-gravestone':
				{
						defaultCamZoom = 0.9;
						curStage = 'your-gravestone';
						var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('die/bg'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.9, 0.9);
						bg.active = false;
						add(bg);
		...
		switch (curStage)
		{
			...
			bambi = new Character(0, 0, 'bambi');
			spong = new Character(0, 0, 'spong');
			bob = new Character(0, 0, 'bob');
			// Shitty layering but whatev it works LOL
			if (curStage == 'limo')
				add(limo);
			if (curStage == 'your-gravestone')
			    add(bambi);
			    add(boyfriend);
				add(bob);
				add(spong);
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
					   ...
					   /*switch (Math.abs(daNote.noteData))
						...
						if (isbambi)
						{
							bambi.playAnim('sing' + dadsDir[daNote.noteData], true);
							bambi.holdTimer = 0;
						}
						if (isspong)
						{
							spong.playAnim('sing' + dadsDir[daNote.noteData], true);
							spong.holdTimer = 0;
						}	
						if (isbob)
						{
	...
	*/
	...
	function resetCharacters():Void //trole
	{
	  isbf = false;
	  isbambi = false;
	  isbob = false;
	...
	override function stepHit()
	{
		...
		if (curStage == 'your-gravestone' && curStep != stepOfLast) // song events lol
			{
				switch(curStep)
				{
					case 3:
						resetCharacters();
						switchCharacter('spong');
					case 130:
						resetCharacters();
						switchCharacter('bob');
					case 273:
						resetCharacters();
						switchCharacter('bambi');
					case 400:
						resetCharacters();
						switchCharacter('spong');
					case 529:
